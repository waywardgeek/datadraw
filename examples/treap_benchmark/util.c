    ...
    olNode node)
{
    ...
    if(left != olNodeNull) {
        utAssert(olNodeGetPriorityRootNode(node) >= olNodeGetPriorityRootNode(left));
        olVerifyTree(left);
    }
    if(right != olNodeNull) {
        utAssert(olNodeGetPriorityRootNode(node) >= olNodeGetPriorityRootNode(right));
        olVerifyTree(right);
    }
}
    ...
    uint32 depth)
{
    ...
    indent(depth);
    utDebug("%u 0x%x %u\n", olNodeGetKey(node), olNode2Index(node), olNodeGetPriorityRootNode(node));
