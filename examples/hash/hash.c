...
htNode htNodeCreate(
    ...
    int32 y)
{
    htNode node = htGraphFindXYNode(htTheGraph, x, y);
    uint32 length = strlen(name) + 1;

    if(node != htNodeNull) {
        utExit("Node %s is physically on top of node %s", name, htNodeGetName(node));
    }
    node = htGraphFindNode(htTheGraph, name, length);
    if(node != htNodeNull) {
        utExit("Node %s is defined twice", name);
    }
    node = htNodeAlloc();
    ...
    htGraphInsertXYNode(htTheGraph, node);
    return node;
}
